...
- Release audit also enforces exactly one homepage archive-card marker (`Start here · Latest edition`), requires that card link to match the RSS first-item issue slug, and validates hero/card freshness date text against RSS first-item metadata (`Week of …` + full publication date).
